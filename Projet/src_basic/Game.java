...
public class Game extends Application {
	...
	public void start(Stage stage) {
        ...
        root.getChildren.addAll(text, actionClickOK);
        */
		...
		Image imgShip = new Image(Game.getRessourcePathByName("images/ship.png"),30,30 , false, false);
		...
		while (cpt<nbrPlanet) {
			if (cpt==4 ) {
				...
				Circle c = new Circle(Math.random() * 50+50,
						   new Point(100 + Math.abs(Math.random()*WIDTH-200), 100 + Math.abs(Math.random()*HEIGHT-200)));
		...
		AnimationTimer animation = new AnimationTimer() {
			...
			public void handle(long arg0) {
				...
				if(action=="play") {
					...
					while (it.hasNext()) {
						...
						while( it2.hasNext()) {
							...
							if(ship.getPlanetTarget()!=null && ship.validatePosition() && (ship.getPlanetChosen()==plan) ) {
								ship.render(gc, listPlanets, imgShip);
					...
					String s = Planet.end(listPlanets);
					if(s=="You lose") {
						gc.drawImage(loose, 0, 0);
						gc.fillText(s, 100, 200);
						this.stop();
					}
					if(s=="VICTORY" ) {
						gc.drawImage(victory, 0, 0);
						gc.fillText(s, 345, 200);
						this.stop();
					}
				    timer=timer+1;
				}
